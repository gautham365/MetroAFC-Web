...
export default function Admin() {
    ...
    const handleUpdate = () => {
        setOpen(false);
        axios.post(process.env.REACT_APP_HOST + "/api/updateRow", { table: Tables[buttonSelected].Tables_in_metroafc, row: dialogData, prevRow: prevDialogData })
            .then((res) => {
                handleTable(Tables[buttonSelected].Tables_in_metroafc);
            })
            .catch((err) => {
                console.log(err);
            });
    ...
    const handleAdd = (e) => {
        e.preventDefault();
        setOpenAdd(false);
        axios.post(process.env.REACT_APP_HOST + "/api/insertRow", { table: Tables[buttonSelected].Tables_in_metroafc, row: dialogData })
            .then((res) => {
                handleTable(Tables[buttonSelected].Tables_in_metroafc);
            })
            .catch((err) => {
    ...
    useEffect(() => {
        axios.get(process.env.REACT_APP_HOST + "/api/getAllTables")
            .then((res) => {
                setTables(res.data);
            })
            .catch((err) => {
    ...
    useEffect(() => {
        if (Tables.length > 0) {
            axios.post(process.env.REACT_APP_HOST + "/api/getTable", { table: Tables[buttonSelected].Tables_in_metroafc })
                .then((res) => {
    ...
    const handleTable = (table) => {
        axios.post(process.env.REACT_APP_HOST + "/api/getTable", { table })
            .then((res) => {
                setdataTable(res.data);
            })
            .catch((err) => {
        ...
        <div style={{ height: "100vh", display: "flex", gap: "3px" }}>
            ...
            >
                ...
                >
                    ...
                    {Tables.map(({ Tables_in_metroafc }, index) =>
                    (<Button
                        key={index}
            ...
            <Box sx={{
                ...
                <Button
                    variant="contained"
                    sx={{ position: "absolute", right: "4vh", top: "3vh" , minWidth:"80px"}}
                    onClick={() => {
                        setOpenQuery(true);
                    }}
                    >
                                        ...
                                        <StyledTableCell align="right" key={'action'}>
                                            <div style={{ display: "flex", gap: "10px", justifyContent: "right" }}>
                                                ...
                                                {/* <IconButton aria-label="delete" sx={{ marginTop: "6px" }}
                                                    onClick={() => {
                                                        axios.post(process.env.REACT_APP_HOST + "/api/deleteRow", { table: Tables[buttonSelected].Tables_in_metroafc, row })
                                                            .then((res) => {
